...
from litetype.backends.macos import insert_text
...
class TestBasic:
    ...
    def test_successful_insert_returns_true(self):
        ...
        with patch('litetype.backends.macos.subprocess.Popen', return_value=_popen()), \
             patch('litetype.backends.macos.subprocess.run', side_effect=runs):
            assert insert_text("hello") is True

    def test_pbcopy_failure_returns_false(self):
        runs = iter([_run(stdout=b'')])               # pbpaste
        with patch('litetype.backends.macos.subprocess.Popen', return_value=_popen(returncode=1)), \
             patch('litetype.backends.macos.subprocess.run', side_effect=runs):
...
class TestClipboardRestore:
    def test_text_encoded_utf8_into_clipboard(self):
        ...
        with patch('litetype.backends.macos.subprocess.Popen', return_value=popen_mock), \
             patch('litetype.backends.macos.subprocess.run', side_effect=runs):
            insert_text("héllo")
    ...
    def test_original_clipboard_restored_after_paste(self):
        ...
        with patch('litetype.backends.macos.subprocess.Popen', side_effect=mock_popen), \
             patch('litetype.backends.macos.subprocess.run', side_effect=runs):
    ...
    def test_no_restore_when_pbpaste_raises(self, capsys):
        ...
        with patch('litetype.backends.macos.subprocess.Popen', side_effect=mock_popen), \
             patch('litetype.backends.macos.subprocess.run', side_effect=mock_run):
