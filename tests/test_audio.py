"""Tests for microphone capture and recovery logic in vtt.audio.AudioRecorder."""

import numpy as np
import pytest
from unittest.mock import MagicMock, patch

from vtt.audio import AudioRecorder
...
class TestStartRecordingRetry:
    def test_opens_successfully_on_first_try(self):
        ...
        with patch('vtt.audio.sd.query_devices', return_value={'default_samplerate': 16000}), \
             patch('vtt.audio.sd.InputStream', return_value=mock_stream) as mock_input_stream, \
             patch('vtt.audio.sd._terminate') as mock_terminate:
...
class TestStartStopIntegration:
    def test_stop_recording_resamples_using_the_rate_start_recording_opened_at(self):
        recorder = AudioRecorder()
        mock_stream = MagicMock()
        with patch('vtt.audio.sd.query_devices', return_value={'default_samplerate': 48000}), \
             patch('vtt.audio.sd.InputStream', return_value=mock_stream):
            recorder.start_recording()
